...
public class BoardManager : MonoBehaviour
{
    ...
    private IEnumerator ClearMatches(List<TileController> matcthingTiles, System.Action onCompleted)
    {
        List<bool> isCompleted = new List<bool>();

        for (int i = 0; i < matcthingTiles.Count; i++)
        {
        ...
        {
            int index = i;
            StartCoroutine(matcthingTiles[i].SetDestroyed(() => {
    ...
    {
        StartCoroutine(RepositionTiles(ProcessMatched));
    }
